...
let fields = [
	['title', settings({help_text: 'E.g. Maintenance01'})], 
	['equipment', settings({help_text: 'Equipment ID in the database'})], 
	['description', settings({required_view: false})],
	['location', settings({required_view: false})],
	['company', settings({required_view: false, help_text: 'Name of company providing the service'})],
	['operator', settings({required_view: false, help_text: 'Name of the operator/s'})],
    ['start_time', settings({required_view: false, field_type: 'timestamp'})],
	['end_time', settings({required_view: false, field_type: 'timestamp'})],
	['cost', settings({required_view: false, help_text: 'The overall cost of the service'})],
	['supp_kvp', settings({default_value: '{}', field_type: 'JSON', required_view: false, help_text: 'E.g. {"Installed" : true, "Size": 100}'})],
	['supp_file', settings({required_view: false, help_text: 'File ID in FileUpload table'})],
...
export function List () {
	...
	const exclude = ['description', 'location', 'company', 'operator', 'end_time', 'supp_kvp', 'supp_file', 'upload_file'];
	const include_fields = fields.filter(field => !exclude.includes(field[0]));	
